...
async function getAuthToken(user) {
  return new Promise((resolve, reject) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode === 200 && response.accessToken) {
            ...
            console.log(`❌ ${user.username} login failed:`, response);
            resolve(null);
          }
        } catch (error) {
          console.log(`❌ ${user.username} parse error:`, error.message);
          resolve(null);
        }
      });
    });
    ...
    req.on('error', (error) => {
      console.log(`❌ ${user.username} request error:`, error.message);
      resolve(null);
    });
